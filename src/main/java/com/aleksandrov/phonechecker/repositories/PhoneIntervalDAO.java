...
public interface PhoneIntervalDAO extends JpaRepository<PhoneInterval, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT PHONE_INTERVAL.ID, " +
            "PHONE_INTERVAL.END_INTERVAL, PHONE_INTERVAL.PREFIX, " +
            "PHONE_INTERVAL.START_INTERVAL, PHONE_INTERVAL.OPERATOR_ID, " +
            "PHONE_INTERVAL.REGION_ID FROM PHONE_INTERVAL " +
            ...
            "WHERE PHONE_INTERVAL.PREFIX = ? AND PHONE_INTERVAL.START_INTERVAL <= ? " +
            "AND PHONE_INTERVAL.END_INTERVAL >= ? LIMIT 1")
